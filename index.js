 ...
 */
export default async (targetElement, threshold = 0) => new Promise((resolve, reject) => {
    if (!targetElement) {
        return reject(new Error('Must specify a target element.'));
    }

    if (!isThresholdValid(threshold)) {
        return reject(new Error('Threshold must be between 0 and 1 inclusive.'));
    }
